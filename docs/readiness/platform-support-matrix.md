...
|---|---|---|---|---|
| Android | **Internal beta** | Change 2 (`wire-android-production-runtime`) complete: real session/credential adapter, authenticated clients, wired ViewModels, single authenticated start destination | Change 2 | Done. `AndroidSessionState` — a typealias over the shared `ValidatingSessionState` (`:core:data`, since PR #235; Keystore-backed credential store, HMAC profile-key derivation, cold-start recovery, typed auth errors) replaces `InMemorySessionState` on Android's production path; `SubSlothNavHost` starts at `CatalogKey`; library/downloads/settings/player/detail/auth-repair all wired to real adapters. Stream-source resolution is production-wired since PR #234: `ApiPlaybackPort` resolves signed stream URLs for movies, episodes, and shows. One known gap: the movie/series detail entries in `SubSlothNavHost` do not pass `onPlayClick`, so Play from a detail screen is a no-op on Android (flagged during PR #235 review; desktop wires it, Android does not). |
...
| Desktop | Preview → next tier | Downloads wired on desktop (JVM storage shell behind `DownloadController`'s `Context`-backed collaborators) | Follow-up to PR #235 | `DownloadController` and its storage/`ConnectivityChecker` shell are androidMain-only; desktop `DownloadsViewModel`/download controls stay on safe empty/no-op defaults |
