...
| Android | Internal beta → next tier | Required platform tests green (Change 5) | Change 5 | CI job "📱 Instrumented" runs (93 tests as of Change 2, including new session/library/account-switching coverage); TV focus traversal tests now exist (`TvFocusTraversalTest` consumes `testing/tv-focus-harness`: initial focus plus D-pad traversal into media cards), and TV screens get overscan-safe padding via `LocalIsTelevision`/`tvSafeHorizontalPadding` |
...
| TV focus traversal/restoration tests | Traversal covered: `TvFocusTraversalTest` runs in the Instrumented job and consumes `testing/tv-focus-harness`; explicit focus restoration still relies on Compose's default focus state holder | Focus restoration assertions remain owned by Change 5 |
