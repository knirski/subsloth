...
| Android | Internal beta → next tier | Required platform tests green (Change 5) | Change 5 | CI job "📱 Instrumented" runs (93 tests as of Change 2, including new session/library/account-switching coverage); TV initial focus is covered by `TvFocusDesktopTest` (the headless CI emulator cannot grant window focus, so `TvFocusTraversalTest` is an on-device recipe), and TV screens get overscan-safe padding via `LocalIsTelevision`/`tvSafeHorizontalPadding` |
...
| Desktop | **Preview** | Real composition root and routes replacing placeholders and in-memory session (Change 3A) | Change 3A | Done by PR #235. `DesktopContainer` wires DataStore preferences, the machine-keyed credential store, Room, and `ValidatingSessionState` (cold-start recovery); the session-scoped `Api`/`CatalogRepository`/`ApiPlaybackPort` trio rebuilds on session change; all nav routes are wired (catalog, detail, player, library, settings, diagnostics, auth repair, offline library). Tier stays Preview — remaining gaps below. |
| Desktop | Preview → next tier | Downloads wired on desktop (JVM storage shell behind `DownloadController`'s `Context`-backed collaborators) | Follow-up to PR #235 | Done. `DownloadController` (moved to `:core:media` commonMain — platform-neutral) runs with `DesktopDownloadStore`/`DesktopStorageProvider`/`DesktopConnectivityChecker` (JVM storage shell in `:core:media` `jvmMain`); `DesktopContainer`/`DesktopNavHost` wire the downloads, library, offline-library, and settings entries. Tier still gated by the rows below. Byte-transfer runs on both platforms via `DownloadTransferCoordinator` (see `docs/architecture/composition-roots.md`); subtitle byte-transfer is wired, and media transfers resume from a staged partial with ranged requests. |
| Desktop | Preview → next tier | Desktop tests required in CI (Change 5) | Change 5 | Done. CI job "☕ JVM / 🖥️ Desktop" runs `:desktopApp:test` under Xvfb (39 UI tests: login, catalog/detail, library/settings/downloads, player, search, accessibility). |
...
| Web production | **Not yet granted** | Approved auth/CORS architecture; COOP/COEP-capable host; persistence reload test; real adapters; production browser suite (Change 3B) | Change 3B | Real adapters landed: `WebProductionContainer` (localStorage DataStore, browser credential store, OPFS Room, `ValidatingSessionState`, session-scoped `Api`/`CatalogRepository`/`ApiPlaybackPort` with progressive-download preference) behind a build-injected `SUBSLOTH_API_BASE_URL` (dispatch-gated CI artifact job). Still open: COOP/COEP-capable host (Pages lacks the headers → OPFS degrades to in-memory), auth/CORS decision record, persistence-reload test, production browser suite. Playback note: browsers need a progressive `download_url` (no native HLS). |
...
| Android session/data runtime has no in-memory/no-op binding | `AndroidSessionStateInstrumentedTest`/`LogoutCleanupInstrumentedTest` assert `AppContainer.sessionPort is AndroidSessionState` and `!is InMemorySessionState`; production startup wires real session, library, downloads, and settings adapters. Since PR #235, `AndroidSessionState` is a typealias over the shared `ValidatingSessionState` in `:core:data` — the assertions continue to hold unchanged | Change 2, done for session/data adapters |
| Android player stream-source resolution is production-wired | Done (PR #234): `ApiPlaybackPort` in `:core:network` implements `PlaybackPort` and `SubSlothNavHost`'s player entry passes `prepareSource`/`refreshStreamUrl`. Automated check: `ApiPlaybackPortTest` (MockEngine, 9 tests incl. HTTP 401 classification) runs in `:core:network:jvmTest` under the `☕ JVM / 🖥️ Desktop` job. No automated check asserts the nav-host wiring itself (compile-only) | PR #234 |
| Desktop compiles | CI job `☕ JVM / 🖥️ Desktop` (desktop compilation step) | Passing, required |
| Desktop real composition root and routes (no placeholders) | Done (PR #235): `DesktopContainer` wires real adapters and `DesktopNavHost` has no placeholder entries. No automated check yet — no test asserts absence of placeholder navigation or the in-memory session on desktop | PR #235 |
| Desktop unit/UI tests run in CI | CI job `☕ JVM / 🖥️ Desktop` (Desktop tests step): `xvfb-run -a ./gradlew :desktopApp:test`, gated by the desktop/shared path filter (also runs on workflow_dispatch) | Done |
...
| TV focus traversal/restoration tests | Initial focus (`tvInitialFocus`) is covered by `TvFocusDesktopTest` in the JVM/Desktop job; `TvFocusTraversalTest` consumes `testing/tv-focus-harness` for on-device remote-control traversal but is ignored in CI (headless emulator never grants window focus); explicit focus restoration still relies on Compose's default focus state holder | Focus restoration assertions remain owned by Change 5 |
