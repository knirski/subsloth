...
| Desktop playback: `UnsatisfiedLinkError: libgstapp-1.0.so.0: cannot open shared object file` | The player bridge's native library needs GStreamer, which is not on the host library path (NixOS) | Run the app inside the Nix dev shell (`./gradlew :desktopApp:run`) or via the wrapped `nix build .#subsloth` package; both provide the GStreamer libraries, `GST_PLUGIN_SYSTEM_PATH_1_0`, and the GIO TLS backend. Verify with `ldd ~/.cache/composemediaplayer/native/linux-x86-64/libNativeVideoPlayer.so` — it should report no `not found` entries |
| Desktop playback: `GStreamer error: Internal data stream error` / `Stream doesn't contain enough data` | `souphttpsrc` cannot open `https://` streams because the GIO TLS backend (glib-networking) is missing | Same fix as above; the dev shell exports `GIO_EXTRA_MODULES` and the Gradle `run` task forwards it. Re-`direnv reload` after pulling the fix |
| Desktop playback: `Caught a segmentation fault while loading plugin file: …/libgstplayback.so` | The GStreamer plugin scanner crashed while rebuilding a stale/partial plugin registry (for example after changing the native library set) | Remove the registry cache and retry: `rm -rf ~/.cache/gstreamer-1.0` (and `~/.cache/composemediaplayer` to force re-extraction). If it persists, run with `GST_DEBUG=2` and inspect the lines before the crash |
