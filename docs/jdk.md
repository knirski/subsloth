...
- **JDK 17** for the toolchain — what AGP 9.x and the current Kotlin version (see `gradle/libs.versions.toml`) officially target on Android. Desugaring needs are driven by source/target compatibility, API usage, and the AGP/`compileSdk` configuration, not by the toolchain JDK version alone; bumping the toolchain wouldn't change that configuration and buys nothing for the current `compileSdk`/`minSdk` (see `gradle/libs.versions.toml` and the convention plugins in `build-logic/convention/`; do not duplicate the numbers here — they drift).
