...
### 3. ResponseValidationPlugin (custom)
...
### 5. HttpRequestRetry
Bounded retry: 429 and 5xx responses. Max 2 retries with fixed linear backoff (`(attempt + 1) * 500ms`). Also retries on exception/timeout.

### 6. Logging
`LogLevel.HEADERS` (when `enableHttpLogging = true`). Uses `InterceptorLogger` Kermit bridge. `Authorization` header redacted.

### 7. defaultRequest (configuration block)
...
429 responses are retried with fixed linear backoff (`(attempt + 1) * 500ms`, max 2 retries). The implementation does not inspect the `Retry-After` header — all 429 responses are treated equally.
