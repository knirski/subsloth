...
- [ ] 2.2 Implement local-file-first playback and no-network-refresh behavior for offline files.
- [ ] 2.3 Implement bounded stream URL refresh, current-playback quality fallback, and recoverable player errors.
...
- [ ] 3.3 Implement auth failure handling that saves progress and routes to auth repair only for online playback.

## 4. Verification

- [x] 4.1 Run `./gradlew :core:domain:test :core:media:test :feature:player:test :app:assembleDebug`.
- [ ] 4.2 Run manifest/lint checks for `mediaPlayback` foreground-service type.
