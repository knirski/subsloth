...
class AccountProfileStoreTest {
    ...
    @Test
    fun `deriveProfileKey returns same key for same normalized login`() =
        runTest {
    ...
    @Test
    fun `deriveProfileKey returns different keys for different logins`() =
        runTest {
    ...
    @Test
    fun `deriveProfileKey normalizes email case`() =
        runTest {
    ...
    @Test
    fun `deriveProfileKey normalizes leading and trailing whitespace`() =
        runTest {
    ...
    @Test
    fun `deriveProfileKey normalizes Unicode NFC`() =
        runTest {
    ...
    @Test
    fun `raw login is not stored as account identifier`() =
        runTest {
    ...
    @Test
    fun `same salt produces same key for same login`() =
        runTest {
    ...
    @Test
    fun `getOrCreateSalt generates a salt on first call`() =
        runTest {
    ...
    @Test
    fun `deriveProfileKey produces HMAC-SHA256 length output`() =
        runTest {
    ...
    @Test
    fun `salt is not cleared by repeated calls`() =
        runTest {
            val salt1 = store.getOrCreateSalt()
            val salt2 = store.getOrCreateSalt()
            assertThat(salt1).isEqualTo(salt2)
