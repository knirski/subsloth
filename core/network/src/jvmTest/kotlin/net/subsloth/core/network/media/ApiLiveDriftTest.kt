...
class ApiLiveDriftTest {
    ...
    private suspend fun assertHttpError(
        expectedCode: Int,
        block: suspend () -> Unit,
    ) {
        val error =
    ...
    @Test
    fun `list movies returns non-empty typed movie list`() =
        runTest {
    ...
    @Test
    fun `list shows returns non-empty typed show list`() =
        runTest {
    ...
    @Test
    fun `movie detail with valid id returns typed movie`() =
        runTest {
            val firstId =
                api
                    .listMovies()
    ...
    @Test
    fun `movie detail with nonexistent id returns 404`() =
        runTest {
            assertHttpError(404) { api.getMovie(0) }
        }

    @Test
    fun `show detail with valid id returns typed show with episodes`() =
        runTest {
            val firstId =
                api
                    .listShows()
    ...
    @Test
    fun `episode detail with discovered id returns typed episode`() =
        runTest {
            val firstShowId =
                api
                    .listShows()
